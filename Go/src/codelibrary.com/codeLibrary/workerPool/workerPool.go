...
var (
	MaxWorker = 5      // 最大的worker数量
	JobQueue  chan Job // 工作通道，模拟需要处理的所有工作
)

type Worker struct {
	id         int           // id，worker编号
	WorkerPool chan chan Job // 工作者池（通道的通道），每个元素都是一个job通道（JobChannel），公共的job
	JobChannel chan Job      // 工作通道，每个元素是一个job，worker私有的job
	exit       chan bool     // 结束信号，当有数据写入时，worker结束运行
}

// 用于创建一个新的worker，JobChannel和exit变量则是随着Worker的新建而新建的。
func NewWorker(workerPool chan chan Job, id int) Worker {
	fmt.Printf("new a worker (%d) \n", id)
	return Worker{
		id:         id,
		WorkerPool: workerPool, // workerPool和scheduler是同一个
...
func (w Worker) Start() {
	go func() {
		for {
			...
			fmt.Println("register private JobChannel to publice WorkerPool", w)
			select {

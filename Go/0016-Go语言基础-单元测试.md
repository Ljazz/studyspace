...
- [2 测试函数](#2-测试函数)
  ...
  - [2.5 测试覆盖率](#25-测试覆盖率)
- [3 基准测试](#3-基准测试)
  - [3.1 基准测试函数格式](#31-基准测试函数格式)
  - [3.2 基础测试示例](#32-基础测试示例)
  - [3.3 性能比较函数](#33-性能比较函数)
  - [3.4 重置时间](#34-重置时间)
  - [3.5 并行测试](#35-并行测试)
- [4 Setup与TearDown](#4-setup与teardown)
  - [4.1 TestMain](#41-testmain)
  - [4.2 子测试的Setup与Teardown](#42-子测试的setup与teardown)
- [5 示例函数](#5-示例函数)
  - [5.1 示例函数的格式](#51-示例函数的格式)
  - [5.2 示例函数示例](#52-示例函数示例)

<!-- /TOC -->

# 1 Go test工具

Go中的测试依赖`go test`命令。编写测试代码和编写普通的Go代码过程是类似的，并不需要学习新的语法、规则火攻击。

`go test`命令是一个按照一定规定和组织的测试代码的驱动程序。在包目录内，所有以`_test.go`为后缀的源代码文件都是`go test`测试的一部分，不会被`go build`编译到最终的可执行文件中。
...
测试函数的名字必须以`test`开头，可选的后缀名必须以大写字母开头，如下示例
...
一个软件程序是由很多单元组件构成的，单元组件可以是函数、结构体、方法和最终用户可能以来的任何东西。总之我们需要确保这些组件是能够正常运行的。单元测试是一些利用各种方法测试单元组件的程序，它会将结果与预期输出进行比较。
...
```
在`split`包路径下，执行`go test`命令，输出结果
```
split $ go test
PASS
ok      github.com/Q1mi/studygo/code_demo/test_demo/split       0.005s
```
一个测试用例有点单薄，再加一个测试使用多个字符切割字符串的例子
...
```
上述描述信息中可以看出`TestMoreSplit`这个测试没有成功。还可以在`go test`命令后添加`-run`参数，它对应一个正则表达式，只有函数名匹配上的测试函数才会被`go test`命令执行。
...
最初的`split`函数并没有考虑到sep为多个字符的情况，修改bug
```go
package split
...
```
通过上述的代码把多个测试用例合到一起，再次执行`go test`命令。
```
split $ go test -v
=== RUN   TestSplit
...
func TestSplit(t *testing.T) {
    ...
    for name tc := range tests {
        got := Split(tc.input, tc.sep)
        if !reflect.DeepEqual(got, tc.want) {
            t.Errorf("name:%s excepted:%#v, got:%#v", name, tc.want, got)   // 将测试用例的name格式话输出
        }
    }
}
```
上述做法是能够解决问题的，但是我们也可以按照如下方式使用`t.Run`执行子测试
...
func TestSplit(t *testing.T) {
    ...
    for name, tc := range tests { // 使用t.Run()执行子测试
...
其中`BenchmarkSplit-8`表示对Split函数进行基准测试，数字`8`表示`GOMAXPROCS`的值，这个对于并发基准测试很重要。`10000000`和`203ns/op`表示每次调用Split函数耗时`203ns`，这个结果是`10000000`次调用的平均值。
...
```go
func Split(s, sep string) (result []string) {
    result = make([]string, 0, string.Count(s, sep)+1)
    i := strings.Index(s, sep)
    for i > -1 {
        result = append(result, s[:i])
        s = s[i+len(sep):]  // 使用len(sep)获取sep的长度
...
上面的基准测试只能得到给定操作的绝对耗时，但是在很多性能问题是发生在两个不同操作之间的相对耗时。我们通常需要对两个不同算法的实现使用相同的输入来进行基准比较测试。
